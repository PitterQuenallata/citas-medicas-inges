...
<div class="card max-w-2xl p-4 sm:p-5">
    <h3 class="text-base font-medium text-slate-700 dark:text-navy-100 mb-6">Registrar Nuevo Horario</h3>
        ...
        <div class="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <label class="block">
                ...
                <select name="dia_semana" required
                    ...
                    <option value="" disabled {{ old('dia_semana') ? '' : 'selected' }}>Seleccionar...</option>
                    @foreach($dias as $k => $v)
                        <option value="{{ $k }}" {{ (string)old('dia_semana') === (string)$k ? 'selected' : '' }}>{{ $v }}</option>
                    @endforeach
                </select>
            ...
            <label class="block">
                ...
                <input type="number" name="duracion_cita_minutos" min="5" max="480" value="{{ old('duracion_cita_minutos', 30) }}" required

...
<div class="flex items-center justify-between pb-4">
    ...
    <div class="flex items-center gap-2">
        ...
        @if($cita->pago && $cita->pago->estado_pago === 'pagado')
            <a href="{{ route('pagos.recibo', $cita->pago->id_pago) }}" target="_blank" class="btn h-8 rounded-full border border-success px-4 text-xs font-medium text-success hover:bg-success/10">
                <svg class="mr-1 inline size-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                Recibo
            </a>
        @endif

        @if($cita->paciente?->telefono && !in_array($cita->estado_cita, ['cancelada', 'atendida', 'reprogramada']))
            <form method="POST" action="{{ route('notificaciones.enviar', $cita->id_cita) }}" class="swal-whatsapp">
                @csrf
                <button type="submit" class="btn h-8 rounded-full px-4 text-xs font-medium text-white hover:opacity-90" style="background-color: #25D366;">
                    <svg class="mr-1 inline size-3.5" fill="currentColor" viewBox="0 0 24 24"><path d="M17.472 14.382c-.297-.149-1.758-.867-2.03-.967-.273-.099-.471-.148-.67.15-.197.297-.767.966-.94 1.164-.173.199-.347.223-.644.075-.297-.15-1.255-.463-2.39-1.475-.883-.788-1.48-1.761-1.653-2.059-.173-.297-.018-.458.13-.606.134-.133.298-.347.446-.52.149-.174.198-.298.298-.497.099-.198.05-.371-.025-.52-.075-.149-.669-1.612-.916-2.207-.242-.579-.487-.5-.669-.51-.173-.008-.371-.01-.57-.01-.198 0-.52.074-.792.372-.272.297-1.04 1.016-1.04 2.479 0 1.462 1.065 2.875 1.213 3.074.149.198 2.096 3.2 5.077 4.487.709.306 1.262.489 1.694.625.712.227 1.36.195 1.871.118.571-.085 1.758-.719 2.006-1.413.248-.694.248-1.289.173-1.413-.074-.124-.272-.198-.57-.347z"/><path d="M12 0C5.373 0 0 5.373 0 12c0 2.625.846 5.059 2.284 7.034L.789 23.492a.5.5 0 00.61.61l4.458-1.495A11.952 11.952 0 0012 24c6.627 0 12-5.373 12-12S18.627 0 12 0zm0 22c-2.37 0-4.567-.82-6.3-2.188l-.44-.352-3.2 1.073 1.073-3.2-.352-.44A9.955 9.955 0 012 12C2 6.486 6.486 2 12 2s10 4.486 10 10-4.486 10-10 10z"/></svg>
                    WhatsApp
                </button>
            </form>
        @endif
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    document.querySelectorAll('.swal-whatsapp').forEach(form => {
        form.addEventListener('submit', function(e) {
            e.preventDefault();
            Swal.fire({
                title: '¿Enviar WhatsApp?',
                text: 'Se enviará un recordatorio al paciente por WhatsApp',
                icon: 'question',
